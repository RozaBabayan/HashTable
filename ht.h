...
struct Prober {
    ...
    HASH_INDEX_T next() { throw std::logic_error("Not implemented...should use derived class"); }
};
...
public:
    ...
    // To be completed
    HASH_INDEX_T next() {
        size_t newNumProbes = (this->numProbes_ * dhstep_);

        if (this->numProbes_ == this->m_) {
            return this->npos;
        }
        HASH_INDEX_T loc = (this->start_ + newNumProbes) % this->m_;
        this->numProbes_++;
        return loc;
    }
};
...
HashTable<K, V, Prober, Hash, KEqual>::HashTable(
    ...
    mIndex_ = 0;
    count = 0;
    table_.resize(CAPACITIES[mIndex_], nullptr);  // set hashtable size to the smallest prime
}
...
bool HashTable<K, V, Prober, Hash, KEqual>::empty() const {
    ...
    // and false otherwise
    size_t loc = 0;
    while (loc < table_.size()) {
        if (this->table_[loc] != nullptr && this->table_[loc]->deleted == false) {
            return false;
        }
        loc++;
    }
...
size_t HashTable<K, V, Prober, Hash, KEqual>::size() const {
    ...
    for (size_t i = 0; i < table_.size(); i++) {
        if (this->table_[i] != nullptr) {
            if (this->table_[i]->deleted == false) {
                numEL++;
            }
        }
    }
...
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType& p) {
    ...
    // double alpha = (double)count / CAPACITIES[mIndex_];  // check to see if we need to resize
    if (((double)count / CAPACITIES[mIndex_]) >= this->newAlpha) {
        resize();  // need to resize the table since the load factor >=0.4
    }
    HASH_INDEX_T idx = probe(p.first);
    if (idx == npos) {
        throw std::logic_error("Could not find a free location");

    } else if (idx != npos) {
        if (table_[idx] != nullptr) {
            // there is an actual element ->overwrite with the new one
            table_[idx]->item = p;
            table_[idx]->deleted = false;

        } else if (table_[idx] == nullptr) {
            // index is empty we can store a new element
            HashItem* newEl = new HashItem(p);
            table_[idx] = newEl;
...
void HashTable<K, V, Prober, Hash, KEqual>::resize() {
    ...
    mIndex_++;
    if (mIndex_ == 28) {
        throw std::logic_error("no more CAPACITIES exists");
    }
    ...
    table_.assign(CAPACITIES[mIndex_], nullptr);  // initialised the new locations to null

    while (idx < temp.size()) {
        if (temp[idx]->deleted == false) {
            loc = probe(temp[idx]->item.first);
            table_[loc] = temp[idx];
        }
        idx++;
    }
